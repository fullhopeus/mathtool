...
function controller_midi_in(midiEvent,portName)
	if midiEvent[0] == 0xF0 and midiEvent[1] == 0x42 and midiEvent[2] == 0x40 and midiEvent[3] == 0x6E and midiEvent[4] == 0x00 and midiEvent[8] == 0xF7 then
...
--			return {midi={0xB0,0x07,midiEvent[7]}}
		...
		elseif midiEvent[5] == 0x48 then	-- SW: 0..9: <,>,ENTER,HEX LOCK,EXIT,SCENE,MESSAGE,SETTING
			return {midi={0xB0,0x41+midiEvent[6],midiEvent[7]},inport="PORT B"}

		elseif midiEvent[5] == 0x4B then	-- Joystick: X (0-127),Y (0-127)
			if midiEvent[7] >= 0x40 then
				mod = (midiEvent[7] - 0x40) * 2
				if mod == 0x7E then
					mod = 0x7F
				end
				return {midi={0xB0,0x01,mod,0xE0,midiEvent[6],midiEvent[6]},inport="PORT B"}
			else
				mod = midiEvent[7] * 2
				if mod == 0x7E then
					mod = 0x7F
				end
				return {midi={0xB0,0x02,0x7F-mod,0xE0,midiEvent[6],midiEvent[6]},inport="PORT B"}
			end
	...
	-- filter PitchBend, we receive the Joystick via SysEx
	elseif midiEvent[0] == 0xE0 then
		return {midi={}}
	end
	return nil
end
...
function controller_midi_out(midiEvent,name,valueString,color)
		...
		if midiEvent[1] >= 0x41 and midiEvent[1] <= 0x49 then		-- the buttons
			...
			if insideSetup then		-- if we are inside the patch selection, we display the name immediately without a delay
				...
				if midiEvent[1] >= 0x15 and midiEvent[1] <= 0x1C then
					...
					updateObjectsAfterPatchChange[midiEvent[1] - 0x15] = {midi={0xf0,0x42,0x40,0x6E,0x00,0x22, 9, 0x10 + (midiEvent[1] - 0x15), str, 0xf7}, outport='CTRL'}
				...
				elseif midiEvent[1] >= 0x0D and midiEvent[1] <= 0x14 and updateObjectsAfterPatchChange[midiEvent[1] - 0x0D] ~= nil then
					...
					updateObjectsAfterPatchChange[midiEvent[1] - 0x0D] = {midi={0xf0,0x42,0x40,0x6E,0x00,0x22, 9, 0x20 + (midiEvent[1] - 0x0D), str, 0xf7}, outport='CTRL'}
				end
			else
				...
				if midiEvent[1] >= 0x15 and midiEvent[1] <= 0x1C then
					updateObjects[midiEvent[1]] = {midi={0xf0,0x42,0x40,0x6E,0x00,0x22, 9, 0x10 + (midiEvent[1] - 0x15), string.crunch(name, 8), 0xf7}, outport='CTRL'}
					...
					currentLCDValues[0x10 + (midiEvent[1] - 0x15)] = str
					return {midi={0xf0,0x42,0x40,0x6E,0x00,0x22, 9, 0x10 + (midiEvent[1] - 0x15), str, 0xf7}, outport='CTRL'}
				elseif midiEvent[1] >= 0x0D and midiEvent[1] <= 0x14 then
					updateObjects[midiEvent[1]] = {midi={0xf0,0x42,0x40,0x6E,0x00,0x22, 9, 0x20 + (midiEvent[1] - 0x0D), string.crunch(name, 8), 0xf7}, outport='CTRL'}
					...
					currentLCDValues[0x20 + (midiEvent[1] - 0x0D)] = str
					return {midi={0xf0,0x42,0x40,0x6E,0x00,0x22, 9, 0x20 + (midiEvent[1] - 0x0D), str, 0xf7}, outport='CTRL'}
				end
			end
		end
	elseif midiEvent[0] == 0xF8 then		-- MIDI Beat Clock
		...
		if beatCounter == 1 then
			-- TEMPO LED on
			return {midi={0xF8, 0xf0,0x42,0x40,0x6E,0x00,0x01, 0x16, 0x20, 0xf7}, outport='CTRL'}
		elseif beatCounter == 2 then
			-- TEMPO LED off
			return {midi={0xF8, 0xf0,0x42,0x40,0x6E,0x00,0x01, 0x16, 0x00, 0xf7}, outport='CTRL'}
		end
	...
	elseif midiEvent[0] == 0x99 then		-- MIDI Channel #10 (Drum pads)
		...
		if midiEvent[2] == 0x00 then
			return {midi={0xf0,0x42,0x40,0x6E,0x00,0x01, 0x00 + (midiEvent[1] - 0x3C), 0x00, 0xf7}, outport='CTRL'}
		else
			return {midi={0xf0,0x42,0x40,0x6E,0x00,0x01, 0x00 + (midiEvent[1] - 0x3C), 0x20, 0xf7}, outport='CTRL'}
		end
	end
...
function controller_initialize(applicationName,deviceNewlyDetected)
	...
	if deviceNewlyDetected then
		return {midi={ -- enable native mode
					   ...
					   0xf0,0x42,0x40,0x6E,0x00,0x3F,0x27,0x00, 0x00,	-- Transpose
					   ...
					   0xf0,0x42,0x40,0x6E,0x00,0x3F,0x12,0x01, 0x00,0x00,0x00,0x00,	-- PAD and Main LEDs
																0x00,0x00,0x00,0x00,	-- SubLCD colors
																0x00,					-- Main LCD color
																'MainStge',				-- Main LCD text
																0xF7,
					   -- Native KORG mode Packet Communication 3rd Data (LCD 1..4)
					   0xf0,0x42,0x40,0x6E,0x00,0x3F,0x21,0x02, ' Native ', ' Native ', ' Native ', ' Native ', 0xF7,
					   -- Native KORG mode Packet Communication 4th Data (LCD 5..8)
					   0xf0,0x42,0x40,0x6E,0x00,0x3F,0x21,0x03, ' Native ', ' Native ', ' Native ', ' Native ', 0xF7,
					 }, outport='CTRL'}
	else
		return {midi={ -- enable native mode
					   0xf0,0x42,0x40,0x6E,0x00,0x00,0x00,0x01,0xf7,
...
function controller_finalize()
	return {midi={ -- disable native mode
				   0xf0,0x42,0x40,0x6E,0x00,0x00,0x00,0x00,0xf7
				 }, outport='CTRL'}
end
...
function controller_select_patch(programchangeNumber, patchname, setname, concertname)
	...
	for index=0,7 do
		updateObjectsAfterPatchChange[index] = {midi={0xf0,0x42,0x40,0x6E,0x00,0x22, 9, 0x00 + index, "        ", 0xf7}, outport='CTRL'}
	end

	-- display the patch name in the Main LCD
	return {midi={0xf0,0x42,0x40,0x6E,0x00,0x22, 9, 0x38, string.crunch(patchname, 8), 0xf7}, outport='CTRL'}
end
...
function controller_names(channel)
	if channel == 0 then
		return { [1] = "Modulation", [2] = "Down Modulation",
				 ...
				 [67] = "ENTER", [68] = "HEX LOCK", [69] = "EXIT", [70] = "SCENE", [71] = "MESSAGE", [72] = "SETTING",
				}
	else
		return nil
	end
...
function controller_info()
	return {
		...
		device_inquiry = {0xF0,0x7E,0x00,0x06,0x02,0x42,0x6E,0x00,0x00,0x00,MIDI_Wildcard,MIDI_Wildcard,MIDI_Wildcard,MIDI_Wildcard,0xF7},
		...
		items = {
				...
				{name='Keyboard', label='microKONTROL', objectType='Keyboard', midiType='Keyboard', startKey=36, numberKeys=37, midi={0x90,MIDI_Wildcard,MIDI_Wildcard}, inport="PORT B"},
